...
openBedtimeTale?.addEventListener('click',()=>openBedtimeWorld(false));
...
const sleepMemoryTexts=[
  'Başını kucağıma koyduğunda bütün dünya biraz daha sessiz olurdu.',
...
const sleepMemoryWhispers=['“Buradayım…”','“Gözlerini kapat yavrum…”','“Uyuyana kadar gitmeyeceğim…”','“İyi geceler güzelim…”'];
...
(()=>{
 ...
 document.querySelector('#lyricsFinalOpen')?.addEventListener('click',()=>{
   ...
   document.querySelector('#finalScreen')?.classList.add('show');
 });
...
(()=>{
 ...
 document.querySelector('#ourBeginning')?.addEventListener('click',()=>{
  ...
  show(`<div class="emotion-sheet beginning-sheet">
    ...
    <div class="beginning-line">
      ...
      <article><span>04</span><div><small>HAYATIMIN BİR PARÇASI OLDUN</small><p>Bir süre sonra seninle konuşmak, günümün içindeki bir an olmaktan çıktı; günümün kendisine dönüştü. Uyandığımda aklıma gelen, güzel bir şey olduğunda anlatmak istediğim, yorulduğumda sesini duymayı aradığım kişi sendin. Seni uyuturken huzur buldum.</p></div></article>
    </div>
 ...
 document.querySelector('#myPromises')?.addEventListener('click',()=>{
  const promises=[
    ...
    'Birlikte geçirdiğimiz hiçbir günü sıradan saymayacak, seni seçtiğimi davranışlarımla da göstermeye devam edeceğim.'
  ];
 ...
 document.querySelector('#cleanPolaroid')?.addEventListener('click',()=>{
  const words=[
    ['Yavrum','Sana “yavrum” diye seslendiğimde, içimde sana karşı duyduğum bütün şefkat tek bir kelimeye sığıyor.'],
    ...
    ['Bitanem','Bu kelime benim için yalnızca bir hitap değil; hayatımda kimsenin dolduramayacağı yerin adı.'],
    ['Sesin','Bazen telefon kapandıktan sonra bile gülüşün kulağımda kalıyor ve fark etmeden beni gülümsetmeye devam ediyor.'],
    ['Biz','Aynı anda aynı şeyi düşünüp aynı cümleyi kurduğumuzda, aklımızın bile birbirine benzediğini hatırlatan kelime.']
  ];
